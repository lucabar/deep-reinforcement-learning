...
from Helper import make_tensor, e_greedy, softmax, linear_anneal, smooth, LearningCurvePlot
...
if __name__ == "__main__":
    ...
    Plot = LearningCurvePlot(title=f'Experiment with different epsilons')
            ...
            for arch in architectures:
                for target_update_freq in freqs:
                    for replay_buffer_size in buffer_sizes:
                        ...
                        all_rewards = np.mean((all_rewards), axis=0)
                        np.save(f"runs/book/experiment_{epsilon}", all_rewards)
        learning_curve = smooth(all_rewards, 90)
        Plot.add_curve(
            learning_curve, label=f'epsilon = {str(epsilon)}')

    Plot.save('epsilons.pdf')
