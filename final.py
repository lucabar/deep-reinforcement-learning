...
class Q_Network():
        ...
        if architecture == 1:
            ...
            self.model = tf.keras.Sequential([
                tf.keras.layers.Dense(64, activation=activ, input_shape=(
                    4,), kernel_initializer=init),
                tf.keras.layers.Dense(
                    32, activation=activ, kernel_initializer=init),
                tf.keras.layers.Dense(2)
            ])
        elif architecture == 4:
            self.model = tf.keras.Sequential([
                ...
                tf.keras.layers.Dropout(0.2),
                tf.keras.layers.Dense(2, activation=activ)
            ])
    ...
    def update(self, states, actions, rewards, dones, target_output):
        ...
        # history = self.model.fit(states, target_Q_values,
...
class DQN_Agent():
    ...
    def draw_action(self, state, q_network: Q_Network):
        ...
        Q_vals = q_network.model.predict(
...
def learning(eps, learning_rate, batch_size, architecture, target_update_freq, replay_buffer_size, path_to_weights: str = None):
    ...
    network = Q_Network(learning_rate, optimizer='adam',
                        architecture=architecture, path_to_weights=path_to_weights)
    target = Q_Network()
    target.model = tf.keras.models.clone_model(network.model)
    target.model.set_weights(network.model.get_weights())
    agent = DQN_Agent(batch_size, replay=True, epsilon=1,
                      max_buffer=replay_buffer_size)
    ...
    for episode in range(eps):
        ...
        agent.epsilon = linear_anneal(
            episode, eps, agent.epsilon_initial, 0.001, 0.9)
        ...
        for step in range(475):
            ...
            agent.buffer_update(state, action, reward, next_state, done or trunc)
            state = next_state
...
if __name__ == "__main__":
    ...
    for run in range(n_runs):
        rewards = learning(eps, learning_rate, batch_size, architecture=arch,
