...
class Catch():
    ...
    def __init__(self, rows: int = 7, columns: int = 7, speed: float = 1.0,
        ...
        if self.observation_type == 'vector':  # 1st columns for paddle, 2nd col and rows for lowest ball?
    ...
    def reset(self):
        ...
        # initialized problem
        self.paddle_xy = [self.columns // 2, 0] # paddle in the bottom middle, why does paddle have a y?
    ...
    def _drop_new_ball(self):
        ...
        self.balls_xy.append([self._rng.randint(self.columns),self.rows-1])#0])  x-y coord of ball when dropped
            
    def _get_state(self):
        ''' Returns the current agent observation '''
        if self.observation_type == 'vector': 
            if len(self.balls_xy) > 0: # balls present
                s = np.append(self.paddle_xy[0],self.balls_xy[0]).astype('float32') # paddle X and ball XY
            else:
