...
class Actor():
    def __init__(self, learning_rate: float = 0.01, arch: int = 1, observation_type: str = "pixel",
        ...
        self.ppo = ppo # if true, it is a ppo network
        self.clip_pram = 0.8 # clipping parameter for ppo
    ...
    def update_weights(self, memories):
        ...
        for k, memory in enumerate(memories):
            ...
            states, actions, rewards, values, old_probs = [np.array([experience[field_index]
    ...
    while ep < n_episodes:
        ...
        for m in range(minibatch):
            ...
            for T in range(max_steps):
                ...
                memory[m].append((tf.squeeze(state), action, r, value, action_p))
...
if __name__ == '__main__':
    ...
    minibatch = 2  # but only 2 best ones are used 
    P_weights = None
    V_weights = None
    baseline = True
    eta = 0.001
    ...
    for i in range(3):
        ...
        rewards = reinforce(n_episodes, learning_rate, rows, columns, obs_type,
                            ...
                            P_weights, V_weights, minibatch, eta, stamp, baseline, training, ppo)
